document.addEventListener("DOMContentLoaded", function () {
  ...
  calculateButton.addEventListener("click", function () {
    ...
    resultDisplay.textContent = `/playsound minecraft:block.note_block.harp record @p ~ ~ ~ 3.0 ${newPlaysound.toFixed(9)}`;

    // 履歴アイテムとコピー用ボタンを作成
    const historyItem = document.createElement("li");
    const commandText = `/playsound minecraft:block.note_block.harp record @p ~ ~ ~ 3.0 ${newPlaysound.toFixed(9)}`;
